...
export async function linkReleasedSspsToSubmissions(input: {
  ...
  for (const mDoc of releasedManifestSsps) {
    ...
    await db.transaction(async (tx) => {
      ...
      // Promote the parent SSP document to 'signed' now that its
      // submission has been formally released by QMS. The QMS 4-seat
      // SoD release chain is the authoritative signing event.
      await tx
        .update(sspDocuments)
        .set({ status: "signed", updatedAt: new Date() })
        .where(
          and(
            eq(sspDocuments.id, target.sspDocumentId),
            eq(sspDocuments.status, "draft"),
          ),
        );
  ...
  if (result.released > 0) {
    ...
    if (releasedRows.length > 1) {
      ...
      if (winner && losers.length > 0) {
        ...
        const loserDocIds = losers.map((l) => l.sspDocumentId);
        await db
          .update(sspDocControlSubmissions)
          .set({
            status: "superseded",
        ...
        // Mirror on the parent ssp_documents rows so the SSP version
        // list shows 'superseded' rather than 'effective' for old versions.
        await db
          .update(sspDocuments)
          ...
          .where(
            and(
              ...
              inArray(sspDocuments.id, loserDocIds),
            ),
          );
