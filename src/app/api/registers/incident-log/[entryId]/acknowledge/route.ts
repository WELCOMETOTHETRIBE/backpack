...
/**
 * POST /api/registers/incident-log/[entryId]/acknowledge
 *
 * Admin records the investigation outcome for an auto-detected Microsoft
 * Defender for Endpoint critical/high alert. Validates the entry exists,
 * is type=defender_alert_acknowledgment, status=draft, and belongs to the
 * caller's org. Merges submitted §1 fields into entryData and flips
 * status=final / lifecycle_state=admin_signed.
 *
 * Required fields: business_justification (≥50 chars; doubles as the
 * investigation summary), outcome, actions_taken, signed_at.
 *
 * Optional: ticket_url, notes.
 *
 * SLA: admin should ack within 24h of detection (vs. 72h on lower-severity
 * surfaces). Overdue alerts roll into the next ISSO weekly review as
 * `overdue_escalated`.
 *
 * Auth: session (Admin or Compliance role required).
 *
 * Audit-log chain: enclavewatch.defender_alert.detected (handler) →
 * enclavewatch.defender_alert.admin_acknowledged (here) →
 * enclavewatch.defender_alert.ack_review_applied (next ISSO export).
 *
...
const REQUIRED_STRING_FIELDS: Array<keyof AcknowledgeBody> = [
...
const OUTCOME_ENUM = new Set([
  ...
  "true_positive_in_progress",
  "false_positive_investigated",
  "risk_accepted",
]);
...
export async function POST(
  ...
  if (userRole !== "Admin" && userRole !== "Compliance") {
    return NextResponse.json(
      {
        error:
          "Forbidden — only Admin or Compliance roles may acknowledge Defender alerts",
      },
      { status: 403 },
    );
  }
  ...
  if ((body.business_justification ?? "").trim().length < 50) {
    return NextResponse.json(
      {
        error:
          "business_justification (investigation summary) must be at least 50 characters",
      },
      { status: 400 },
    );
  }
  ...
  if (row.entryType !== "defender_alert_acknowledgment") {
    return NextResponse.json(
      {
        error: `Entry type mismatch — expected "defender_alert_acknowledgment", got "${row.entryType}"`,
      },
      { status: 400 },
    );
  }
  ...
  const evidenceRefs: Array<Record<string, unknown>> = [
    ...existingRefs,
    {
      ...
      value: userId,
      label: `Acknowledged by ${userName}${userEmail ? ` (${userEmail})` : ""} at ${
  ...
  if (body.ticket_url) {
    evidenceRefs.push({
      ...
      label: "Ticket linked at acknowledgment time",
    });
  }
  ...
  console.log(
    JSON.stringify({
      event: "enclavewatch.defender_alert.admin_acknowledged",
      orgId,
      entryId,
  ...
  try {
    await writeAuditLog({
      ...
      userId,
      action: "enclavewatch.defender_alert.admin_acknowledged",
      resourceType: "defender_alert",
      resourceId: (detectionData.alert_id as string | undefined) ?? entryId,
      details: {
        entry_id: entryId,
        alert_title: detectionData.actor_alert_title ?? null,
        severity: detectionData.severity ?? null,
        acknowledged_by: userName,
        outcome: body.outcome,
        ticket_url: body.ticket_url ?? null,
        signed_at: body.signed_at,
        system: detectionData.system ?? null,
        graph_alert_url: detectionData.graph_alert_url ?? null,
      },
    });
  } catch {
    // No-op
