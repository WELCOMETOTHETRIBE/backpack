...
import { db } from "@/db";
import { poamItems, controlImplementations, controls } from "@/db/schema";
...
export default async function PoamPage() {
  ...
  return (
    <div>
      ...
      {items.length === 0 && (
        <p className="text-zinc-500">No POA&M items yet. Create one from a control in POA&M status.</p>
      )}
    </div>
  );
}
