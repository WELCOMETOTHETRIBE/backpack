...
export function DashboardSetupWidget({
  ...
  // Determine the first incomplete stage — that's the "active" stage
  const activeStageIndex = stages?.findIndex((s) => s.items.some((i) => !i.done)) ?? -1;
  ...
  return (
    <>
        ...
        <div>
          <h1 className="text-2xl font-semibold text-[#0F172A] mb-1">
            {onboardingStarted
              ? allDone
                ? "Assessor-ready"
                : "CMMC Certification Journey"
              : "Welcome to CMMC Codex"}
          </h1>
          <p className="text-[15px] text-slate-600">
            {onboardingStarted
              ? allDone
                ? "Your CMMC Level 2 evidence package is complete. You are ready for C3PAO assessment."
                : `${completedCount} of ${totalCount} milestones complete — ${totalCount - completedCount} remaining until you are assessor-ready.`
      ...
      {/* Staged Certification Journey — shown after onboarding starts */}
      {onboardingStarted && stages && !allDone && (
...
function StageGroup({
  ...
  // Active stage is open by default; others are collapsed
