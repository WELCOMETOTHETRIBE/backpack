...
export const REGISTER_DEFINITIONS: { registerKey: string; name: string; description?: string; requiredColumns: RegisterColumn[]; retainForDays?: number }[] = [
  ...
  { registerKey: "technical_compliance_run", name: "Technical Compliance Log", description: "One entry per OS/WSH Collector run. Auto-seeded by EnclaveWatch — records collector name, version, and pass/fail counts as a continuous evidence trail for §3.4.1, §3.14.1, and ongoing conmon obligations.", requiredColumns: [{ key: "run_id", label: "Run ID", type: "string" }, { key: "collector_name", label: "Collector", type: "string" }, { key: "collector_version", label: "Version", type: "string" }, { key: "checks_total", label: "Checks Total", type: "number" }, { key: "checks_passed", label: "Checks Passed", type: "number" }, { key: "checks_failed", label: "Checks Failed", type: "number" }], retainForDays: 365 * 3 },
];
